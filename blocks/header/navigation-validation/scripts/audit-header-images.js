...
/* eslint-disable no-restricted-syntax, no-continue, no-plusplus, no-cond-assign, no-empty, max-len */

/*
 * audit-header-images.js — Node.js CLI tool (not browser code).
 *
 * Three modes:
 *   1) NAV MODE: compare expected images (phase files) vs actual (nav + disk).
 *   2) URL MODE: produce image manifest from live page via Playwright.
 *   3) COMPARE MODE: source vs migrated manifest parity check.
 *
...
function buildExpectedSlots(validationDirAbs) {
  ...
  // When megamenu-mapping has individual items, skip abstract p3 category slots
  // (they're summaries already covered by per-item slots below)
  const hasIndividualItems = !!(mm?.navTriggers);
  if (p3 && !hasIndividualItems) {
    if (p3.hasImages) {
  ...
  if (mm?.navTriggers) {
    for (const trigger of mm.navTriggers) {
      ...
      // Check if a panelItem already covers the featured area (type=featured-card)
      const hasFeaturedCardItem = (trigger.panelItems || []).some(
        (it) => it.type === 'featured-card' && it.hasImage,
      );
      if (trigger.featuredArea?.exists && !hasFeaturedCardItem
        && (trigger.featuredArea?.type || trigger.panelLayout)) {
        slots.push({
  ...
  try {
    ...
    const manifest = await page.evaluate((baseUrl) => {
      ...
      root.querySelectorAll('img').forEach((el) => {
        ...
        add({
          type: 'img', sourceUrl: full, localFilename: (src.split('/').pop() || '').replace(/\?.*$/, ''), description: (el.getAttribute('alt') || '').slice(0, 80), context: 'header', visible: rect.height > 0 && rect.width > 0, size: `${Math.round(rect.width)}x${Math.round(rect.height)}`,
        });
      });
      root.querySelectorAll('svg').forEach((el) => {
        ...
        add({
          type: 'svg', sourceUrl: '', localFilename: '', description: cls || 'svg', context: 'header', visible: true, size: `${Math.round(rect.width)}x${Math.round(rect.height)}`,
        });
      });
      root.querySelectorAll('*').forEach((el) => {
        ...
        add({
          type: 'background-image', sourceUrl: full, localFilename: (src.split('/').pop() || '').replace(/\?.*$/, ''), description: (el.getAttribute('class') || '').slice(0, 60), context: 'header', visible: true, size: `${Math.round(rect.width)}x${Math.round(rect.height)}`,
        });
      });
      ...
      return {
        ...
        images,
        summary: {
          imgTags, svgTags, bgImages, total: images.length,
        },
      };
    }, url);
    await browser.close();
...
function runCompareMode(comparePath, againstPath, validationDir) {
  ...
  const report = {
    timestamp: new Date().toISOString(), sourceTotal: srcTotal, migratedTotal: migTotal, missingCount: missing.length, missing, passed,
  };
...
function runNavMode(navFile, validationDir) {
  ...
  // Use total image references (including duplicates) for slot coverage:
  // same file can legitimately serve multiple megamenu slots (icon reuse).
  // validatedOnDisk checks that every unique file exists and is non-empty.
  const totalRefs = imagePaths.length;
  const actualCount = totalRefs;
  const passed = missingOrEmpty.length === 0 && totalRefs >= expectedCount;
  const missingByLocation = totalRefs < expectedCount
    ? expectedSlots.slice(totalRefs).map((s) => `${s.location}: ${s.description}`)
    : [];
  ...
  try {
    fs.writeFileSync(
      ...
      JSON.stringify({
        timestamp: new Date().toISOString(), expectedCount, actualCount, validatedOnDisk,
      }),
    );
  } catch (_) { /* ignore */ }
  process.exit(0);
