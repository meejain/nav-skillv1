...
function checkRegister(filePath, keyIds, label, workspaceRoot) {
  ...
  const components = register.components || register.items || [];
  for (const keyId of keyIds) {
    ...
    const validStatuses = ['validated', 'validated-with-caveats', 'accepted-with-caveats', 'passed'];
    if (!validStatuses.includes(entry.status)) {
      issues.push(`${label}: "${keyId}" status="${entry.status}" — must be one of: ${validStatuses.join(', ')}`);
    }
    // Normalize similarity: if value is between 0 and 1, treat as fraction and convert to percentage
    let similarity = entry.lastSimilarity;
    if (typeof similarity === 'number' && similarity > 0 && similarity <= 1) {
      similarity *= 100;
    }
    // Components with documented caveats (e.g. transparent header over hero content) may be below 95%
    const hasCaveats = !!(entry.caveats || entry.notes);
    const effectiveThreshold = hasCaveats ? 80 : SIMILARITY_THRESHOLD;
    if (typeof similarity === 'number' && similarity < effectiveThreshold) {
      issues.push(`${label}: "${keyId}" lastSimilarity=${similarity.toFixed(2)}% — must be >= ${effectiveThreshold}%${hasCaveats ? ' (caveat-adjusted)' : ''}`);
    }
