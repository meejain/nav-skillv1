 ...
 *
 * Mobile Dimensional Validation Gate — runs live DOM
 * measurements (getBoundingClientRect, getComputedStyle)
 * on the rendered mobile nav to catch layout bugs that
 * structural and visual-similarity checks miss
 * (e.g. .nav-list 199px vs viewport 375px).
 *
 * GENERIC: Auto-detects menu structure via semantic HTML
 * (nav, ul, li, a) inside .header.
 * Selectors are overridable via config in
 * runGate(viewportWidth, config).
 *
 ...
 *
 * USAGE:
 *   node scripts/mobile-dimensional-gate.js \
 *     --url=<migrated-url> \
 *     [--validation-dir=<path>] \
 *     [--viewport=375x812] \
 *     [--menu-list=.nav-list]
...
function debugLog(validationDir, level, msg) {
  ...
  const tag = '[SCRIPT:mobile-dimensional-gate]';
...
/**
 * Auto-detects the primary nav list inside .header
 * (nav > ul with most visible li children).
 */
...
/**
 * Walks up from element to stopAt, collecting
 * intermediate containers (for width chain).
 */
 ...
 * Returns { gate, viewport, checks, passed, summary,
 *           detectedSelectors }.
 *
 * @param {number} viewportWidth — e.g. 375
 * @param {object} [config] — optional overrides:
 *   header, menuList, menuItems, linkInItem, chevron,
 *   secondaryList, hamburger, minFontSize, minFontWeight
 */
function runGate(viewportWidth = 375, config = {}) {
  const TOLERANCE = 2;
  ...
  function addCheck(id, label, expected, actual, pass, note) {
    report.checks.push({
      id, label, expected, actual, pass, note: note || '',
    });
    if (!pass) report.passed = false;
  }

  const headerSelector = config.header || '.header, header';
  const headerEl = document.querySelector(headerSelector);
  if (!headerEl) {
    addCheck(
      'header-exists',
      `Header block (${headerSelector}) exists`,
      true,
      false,
      false,
  ...
  if (config.menuList) {
    ...
    report.detectedSelectors.menuList = menuList
      ? `auto-detected: ${elementLabel(menuList)}`
      : 'not found';
  }
  ...
  if (!menuList) {
    addCheck(
      ...
      false,
      false,
      'Could not find a <ul> with <li> menu items inside'
      + ' <nav>. Pass config.menuList to specify manually.',
    );
    report.summary = 'FAIL — menu list not found';
    return report;
  }

  const menuListWidth = Math.round(
    menuList.getBoundingClientRect().width,
  );
  const listPct = Math.round(
    (menuListWidth / viewportWidth) * 100,
  );
  addCheck(
    'menu-list-width',
    ...
    menuListWidth < viewportWidth * WIDTH_RATIO_THRESHOLD
      ? `CRITICAL: list is only ${listPct}% of viewport`
      : '',
  );
  ...
  visibleItems.forEach((item, i) => {
    ...
    addCheck(
      ...
      itemWidth < viewportWidth * WIDTH_RATIO_THRESHOLD
        ? `Item is only ${itemPct}% of viewport width`
        : '',
    );
  });
  ...
  visibleItems.forEach((item, i) => {
    ...
    if (config.chevron) {
      ...
      const candidates = Array.from(item.querySelectorAll(
        ':scope > span, :scope > svg,'
        + ' :scope > i, :scope > a > span',
      ));
    ...
    if (chevron) {
      ...
      addCheck(
        ...
        rightGap >= 40
          ? 'Chevron too far from right edge'
          : '',
      );
    }
  });
  ...
  chain.forEach((el) => {
    ...
    const pct = Math.round((w / viewportWidth) * 100);
    addCheck(
      ...
      w < viewportWidth * WIDTH_RATIO_THRESHOLD
        ? `Container is only ${pct}% of viewport`
        : '',
    );
  });
  ...
  if (visibleItems.length > 0) {
    ...
    if (firstLink) {
      ...
      addCheck(
        ...
        parseFloat(computed.fontSize) < minFont
          ? 'Font size smaller than expected for mobile nav'
          : '',
      );
      ...
      addCheck(
        ...
        parseInt(computed.fontWeight, 10) < minWeight
          ? 'Font weight lighter than expected'
          : '',
      );
    }
  }
  ...
  if (secondaryList) {
    const secWidth = Math.round(
      secondaryList.getBoundingClientRect().width,
    );
    addCheck(
      'secondary-nav-width',
      `Secondary nav (${elementLabel(secondaryList)})`
...
/**
 * Builds a single browser script that defines helpers and
 * runs runGate (for page.evaluate).
 */
...
async function main() {
  ...
  args.forEach((a) => {
    if (a.startsWith('--url=')) url = a.slice(6);
      ...
      menuListSelector = a.slice(12);
    }
  });

  if (!url) {
    console.error(
      'Usage: node scripts/mobile-dimensional-gate.js'
      + ' --url=<migrated-url>'
      + ' [--validation-dir=<path>]'
      + ' [--viewport=375x812]',
    );
    console.error(
      'Example: node scripts/mobile-dimensional-gate.js'
  ...
  const absValidationDir = validationDir
    ? path.resolve(validationDir)
    : null;
  if (absValidationDir) {
    debugLog(
      absValidationDir,
      'START',
      `mobile-dimensional-gate.js — url=${url},`
      + ` viewport=${viewport}`,
    );
  }
  ...
  try {
    ...
    // EDS loads header lazily — wait for nav to appear
    ...
    if (hamburger) {
      ...
      await browser.close();
      if (absValidationDir) {
        debugLog(absValidationDir, 'ERROR', 'Hamburger not found');
      }
      process.exit(2);
    }

    const gateConfig = { minFontWeight: 400 };
    if (menuListSelector) gateConfig.menuList = menuListSelector;
    const browserScript = buildBrowserScript(vw, gateConfig);
    ...
    if (absValidationDir) {
      ...
      const reportPath = path.join(
        mobileDir,
        'mobile-dimensional-gate-report.json',
      );
      fs.writeFileSync(
        reportPath,
